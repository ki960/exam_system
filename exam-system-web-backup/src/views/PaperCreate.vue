...
const handleManualSubmit = async () => {
  ...
  try {
    // 根据是否为编辑模式，调用不同接口
    if (isEditMode.value) {
      await request.put(`/api/admin/paperspapers/${paperId.value}`, paperData)
      ElMessage.success('试卷更新成功')
    } else {
      await request.post('/api/admin/paperspapers', paperData)
...
const handleAiSubmit = async () => {
  ...
  try {
    await request.post('/api/admin/paperspapers/ai', paperData)
...
const loadPaperDataForEdit = async () => {
  try {
    const res = await request.get(`/api/admin/paperspapers/${paperId.value}`)
